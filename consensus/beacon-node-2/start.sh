echo "Starting beacon node 2..."

docker run -d \
  --name cl-beacon-node-2 \
  ...
  --http-web3provider=http://host.docker.internal:8553 \
  --accept-terms-of-use \
...
echo "Importing validator 2 keys..."

docker run --rm \
  -v "$(pwd)/consensus/beacon-node-2/wallet:/wallet" \
  -v "$(pwd)/consensus/beacon-node-2/validator_keys:/validator_keys" \
...
docker run -d \
  ...
  -v "$(pwd)/consensus/beacon-node-2/wallet:/wallet" \
  gcr.io/prysmaticlabs/prysm/validator:v3.1.2 \
  --beacon-rpc-provider=host.docker.internal:4002 \
  --beacon-rpc-gateway-provider=http://host.docker.internal:3502 \
  --accept-terms-of-use \
  --chain-config-file=/config.yml \
  --force-clear-db \
  --wallet-dir=/wallet \
  --wallet-password-file=/wallet/password.txt \
  --graffiti=validator-2